...
Any quiz-facing item may include an optional `data.stimulus` object when the
source material contains a diagram, map extract, table, worked example, or
quoted source that should be shown **above the question prompt** instead of
being flattened into plain text.

Use stimulus especially when the learning task depends on students looking at:

- a compass rose, grid, cross-section, sketch map, or ASCII-style diagram
- a table of definitions, values, comparisons, or worked conversions
- a short quoted source extract or evidence block

Do **not** add stimulus to every item. Use it only when the visual or source
...
- `asciiDiagram` — monospaced diagram or map sketch in `content`
- `mapExtract` — same structure as `asciiDiagram`
- `sourceExtract` — short evidence extract or quote in `content`
- `table` or `dataTable` — use `headers[]` and `rows[][]`
...
Stimulus works particularly well for:

- `fillBlank` items using diagrams, maps, worked examples, or tables
- `categorySort` items where students sort evidence from a source table
- `sequence` items based on a process diagram
- `passage` items that need a short quoted source or data table before questions

If a source includes a useful table or diagram, prefer preserving it in
`data.stimulus` rather than rewriting all of that structure into prose.

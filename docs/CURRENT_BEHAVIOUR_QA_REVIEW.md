...
| Language Arcade pass rule | Language Ladder currently runs one `quiz-hunt` round; it auto-retries the same round until the learner reaches 100% accuracy | `src/react/hooks/useLanguageArcadeSession.js`, `src/react/components/learning/LanguageArcadePhase.jsx` | No | Expose sequence and required accuracy in central config so QA can verify the rule without hardcoding it in specs. |

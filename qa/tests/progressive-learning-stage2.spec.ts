...
const STAGE2_PACK_PATH = path.join(
  process.cwd(),
  "data/ProgressiveLanguagePacks/beta1/stage2/semantic_pack_l2_011_people/pack.json",
);
const stage2Pack = JSON.parse(fs.readFileSync(STAGE2_PACK_PATH, "utf8"));
...
test("@data-sample Stage 2 Chinese lesson shows four vocab choices and segmented builder tiles", async ({ page }) => {
  const errors = collectConsoleErrors(page);
  await openHome(page);
  await goToTab(page, "language");
  ...
  await page.getByTestId("progressive-lesson-select").selectOption("semantic_pack_l2_011_people");
  ...
  for (let guard = 0; guard < (stage2Pack.vocabulary || []).length; guard += 1) {
    ...
    const correct = getProgressiveVocabAnswer(stage2Pack, prompt, "zh");
    expect(correct).toBeTruthy();
    await clickOptionByText(options, correct);
    if (guard < (stage2Pack.vocabulary || []).length - 1) {
      await page.getByTestId("progressive-next-step-button").click();
    }
  }
  ...
  const expectedTiles = stage2Pack.sentenceBuilders[0].translations.zh.tiles;

...
export default function ArenaFrame({ activities, onSelect, stats }: { activities: readonly ArenaActivity[]; onSelect: (activity: ArenaActivity) => void; stats?: { level?: string; accuracy?: string; streak?: string; goal?: string } }) {
  ...
  return (
    <div className="flr-arena-hall" data-testid="chinese-input-arena-hall">
      ...
      {featured && <button type="button" className="flr-arena-featured" onClick={() => onSelect(featured)} data-testid="chinese-input-goalkeeper-featured">
        <span className="flr-arena-featured-copy"><small>RECOMMENDED FOR YOU</small><strong>{featured.label} ⚽</strong><span>{featured.description}</span><b>★ Current Journey available</b></span>

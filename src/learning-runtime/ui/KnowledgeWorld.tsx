...
export default function KnowledgeWorld({ nodes, selectedNode, onSelect, onRegionAction, onLessonSelect, onCloseRegion, actions, onStartCustomAdventure }: { nodes: readonly LearningNode[]; selectedNode?: LearningNode | null; onSelect: (node: LearningNode) => void; onRegionAction?: (action: RegionAction, node: LearningNode) => void; onLessonSelect?: (lesson: { id?: string }) => void; onCloseRegion?: () => void; actions?: readonly RegionAction[]; onStartCustomAdventure?: (nodes: readonly LearningNode[]) => void }) {
  ...
  const region = selectedNode?.metadata?.contentCategory === "input-tools"
    ? { label: "Input Tools", icon: "⌘", copy: "Special keys and advanced input tools" }
    : REGION_LABELS[selectedRegion] || { label: selectedRegion, icon: "✦", copy: "Knowledge region" };
  return (
    <div className="flr-knowledge-world">
      ...
      {!selectedNode && inputToolNodes.length > 0 && <section className="flr-input-tools-region" aria-label="Input Tools category"><header><span aria-hidden="true">⌘</span><div><h4>Input Tools</h4><p>Special keys and advanced Cangjie tools</p></div><strong>Tools</strong></header><div className="flr-region-nodes">{inputToolNodes.map((node) => <button type="button" className={`is-${node.state}`} key={node.id} onClick={() => onSelect(node)} title={`${node.label.en}: ${node.state}`} data-testid={`knowledge-node-${node.id}`}><span lang="zh-Hant">{node.glyph}</span><strong>{String(node.metadata?.key || "")}</strong><small>{node.label.en}</small></button>)}</div></section>}
      {!selectedNode && <div className="flr-custom-adventure"><div><p className="flr-eyebrow">Freedom to choose</p><h4>Build a Custom Adventure</h4><p>Select multiple regions and let the Director mix a practice session.</p></div><div className="flr-custom-region-list">{journeyRegions.map((regionId) => { const region = REGION_LABELS[regionId] || { label: regionId, icon: "✦", copy: "Knowledge region" }; const selected = customRegionIds.includes(regionId); return <button type="button" key={regionId} className={selected ? "is-selected" : ""} aria-pressed={selected} onClick={() => setCustomRegionIds((current) => selected ? current.filter((id) => id !== regionId) : [...current, regionId])}><span aria-hidden="true">{region.icon}</span>{region.label} {selected ? "✓" : ""}</button>; })}</div><button type="button" className="flr-custom-adventure-start" disabled={!customRegionIds.length} onClick={() => onStartCustomAdventure?.(nodes.filter((node) => customRegionIds.includes(node.regionId || "world")))}>Start Custom Adventure</button></div>}
